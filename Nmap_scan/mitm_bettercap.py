        ...
        try:
            if proc.stdout:
                ...
                if line:
                    stripped = line.strip()
                    if stripped and show_output.is_set():
                        if 'dns.spoof' in stripped and 'sending' in stripped:
                            print(f"  {stripped}", flush=True)
                elif proc.poll() is not None:
                    break
        except:
...
def run_dns_spoof(iface="wlan0", target_ip=None, gateway_ip=None, spoof_domain=None, my_ip=None):
    ...
    print(" Waiting for DNS requests from target...")
    print("=" * 50)
    print("Press Enter to stop...\n")
...
def start_php_server(webroot, port=8000):
    cmd = ["php", "-S", f"0.0.0.0:{port}", "-t", webroot]
    proc = subprocess.Popen(
        cmd,
...
def main():
    ...
    while True:
        ...
        if opt == "1":
            ...
            print(f"Starting PHP server on 0.0.0.0:8000...")
            php_proc = start_php_server(str(webroot), port=8000)
